...
if pr==2
    v = VideoWriter([options.cwd,'Heuristic_Method_20170328_2D-9.avi'],'Motion JPEG AVI');
    set(v,'FrameRate',3);
    open(v);
end
%profile on;
...
for i=1:max_iter
    ...
    title_text3 = ['(iter=',num2str(i),') Estimated variation source based on heuristic alg-20170314',10,'(\sigma_{data}=',...
        num2str(options.sigma_data),', \sigma_{alg}=',num2str(options.sigma_alg),', l=',num2str(options.l),...
        ', coef=',num2str(coef),') with symmetric K'];
    scatter_label2d_func(Z_est,title_text3,options.dd,0,options.psize,color,mark)
%     title_text4 = ['(iter=',num2str(i),') Estimated variation source (3D) based on heuristic alg-20170314',10,'(\sigma_{data}=',...
%         num2str(options.sigma_data),', \sigma_{alg}=',num2str(options.sigma_alg),', l=',num2str(options.l),...
%         ', coef=',num2str(coef),') with symmetric K'];
%     scatter_PCA_3d(Z_est,pc1,pc2,pc3,pct,title_text4,color_3D,psize,az,0);
    if pr == 1
    ...
    elseif pr == 2
        img = getframe(gcf);
        writeVideo(v,img.cdata);
        close(gcf);
    end
    
    
    %K_est = Gaussian_Kernel(Z_est(:,[1,2])/diag(sqrt(var(Z_est(:,[1,2]))))*diag(sqrt(var(Z(:,[1,2])))),options.sigma_alg);
    K_est = Gaussian_Kernel_vect(Z_est(:,[1,2]),options);%Just use first 2 coordinates of Z_est
%     K_est = Gaussian_Kernel_vect(Z_est(:,[1,2])/diag(sqrt(var(Z_est(:,[1,2])))),options);%Just use first 2 coordinates of Z_est
%     K_est = Gaussian_Kernel_vect(Z_est(:,1:4)/diag(sqrt(var(Z_est(:,1:4)))),options);%Just use first 2 coordinates of Z_est
    %     K_est = Gaussian_Kernel_vect(Z_est,options);%Use all the coordinates of Z_est
    [err_X,mag_err_X,U_K] = proj_err(K_est,V(:,1:th_X),options);
    K_est = K_est + coef*err_X*err_X';
    if i == i_stop
        delta_Z = Z_est(:,[1,2])-Z_est_old;
        break;
    end
%     K_est = K_est + coef*V(:,1:th_X)*V(:,1:th_X)';
%     if i>1
%         n1=[n1;norm(err_X-err_X_old)];
%         n2=[n2;norm(U_K-U_K_old)];
%         n3=[n3;norm(Z_est(:,[1,2])-Z_est_old)];
%     end
%     err_X_old = err_X;
%     U_K_old = U_K;
    Z_est_old = Z_est(:,[1,2]);
    %K_est = K_est + 10*mag_err_X*err_X*err_X';
    mag_err_X
    sum(sum(err_X.^2))
...
delta_Z_norm = sum(delta_Z.^2,2).^0.5;
Z_norm = sum(Z_est.^2,2).^0.5;
Z_temp = Z_est(:,[1,2])-1.5*diag((delta_Z_norm/min(delta_Z_norm)).^1.5)*diag(Z_norm.^4)*delta_Z;
title_text3 = ['(iter=',num2str(i+1),') Estimated variation source based on heuristic alg-20170314',10,'(\sigma_{data}=',...
        num2str(options.sigma_data),', \sigma_{alg}=',num2str(options.sigma_alg),', l=',num2str(options.l),...
        ', coef=',num2str(coef),') with symmetric K'];
scatter_label2d_func(Z_temp,title_text3,options.dd,0,options.psize,color,mark)

if pr == 1
    saveas(gca,[options.cwd,[name,num2str(i)]],'jpg');
    saveas(gca,[options.cwd,[name,num2str(i)]],'fig');
    close(gcf);
elseif pr == 2
    img = getframe(gcf);
    writeVideo(v,img.cdata);
...
%     
%     %K_est = Gaussian_Kernel(Z_est(:,[1,2])/diag(sqrt(var(Z_est(:,[1,2]))))*diag(sqrt(var(Z(:,[1,2])))),options.sigma_alg);
%     K_est = Gaussian_Kernel_vect(Z_est(:,[1,2]),options);%Just let the variance of each Z_est to be unit
