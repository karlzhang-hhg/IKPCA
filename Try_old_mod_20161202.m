...
for i=1:max_iter
    ...
    temp = (lam1*eye(n)+X_til_est'*X_til_est)\X_til_est';
    Psi_est = temp*K_est*temp';
    ...
    title_text3 = ['(iter=',num2str(i),') Estimated variation source based on old model-20161202',10,'(\sigma_{data}=',...
        num2str(options.sigma_data),', \sigma_{alg}=',num2str(options.sigma_alg),', \lambda_{\Psi}=',num2str(lam1),...
        ') with symmetric K'];
